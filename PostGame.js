function PostGame(win)
{
	...
	const can3stars = !player2Mode && levelNum != -1;
	const flawless = player.hp == player.maxHp && (!player2Mode || player2.hp == player2.maxHp);
	var perfectDone, perfectText;

	if (maxStarsGoal(levelNum) == 'p')
	{
		perfectDone = flawless && comboValue.text + itemsUsed * 25 >= enemy.maxHp;
		perfectText = "pg_g_pc";
	}
	if (maxStarsGoal(levelNum) == 'i')
	{
		perfectDone = flawless && player.bomb.alpha == 1;
		perfectText = "pg_g_ni";
	}
	if (maxStarsGoal(levelNum) == 'c')
	{
		perfectDone = flawless && combo100done;
		perfectText = "pg_g_c1";
	}
	if (maxStarsGoal(levelNum) == '0')
	{
		perfectDone = flawless && killedBullets == 0;
		perfectText = "pg_g_d0";
	}
	if (maxStarsGoal(levelNum) == '1')
	{
		perfectDone = flawless && killedBullets >= 10;
		perfectText = "pg_g_d1";
	}
	if (maxStarsGoal(levelNum) == '2')
	{
		perfectDone = flawless && killedBullets >= 20;
		perfectText = "pg_g_d2";
	}
	if (maxStarsGoal(levelNum) == '3')
	{
		perfectDone = flawless && killedBullets >= 30;
		perfectText = "pg_g_d3";
	}
	...
	this.win1 = new createjs.Text(text.get("pg_g_win"), font, "#fff");
	this.win1.name = "postgame win1";
	this.win1.x = this.x + 115;
	this.win1.y = this.y + 20;

	this.win2 = new createjs.Text(text.get("pg_g_flw"), font, flawless ? "#fff" : "#000");
	...
	if (can3stars)
	{
		this.win3 = new createjs.Text(text.get(perfectText), font, perfectDone ? "#fff" : "#000");
	...
	if (can3stars)
	{
		this.star3 = new createjs.Bitmap(loader.getResult("star " + (perfectDone ? 1 : 2)));
	...
	if (levelNum < codes.length && canNextLevel)
	{
	...
	if (canNextLevel)
	{
		...
		var score = 1 + flawless + perfectDone;
...
PostGame.prototype.getTip = function()
{
	...
	else
	{
		...
		// bullets uniques
		if ((levelNum == 7 || levelNum == 15) && (lastHitShot.imageId == 213 || lastHitShot.imageId == 313))
	...
	if (levelNum == 3)
		messages.push("The water is wet.");
	if (levelNum == 15)
